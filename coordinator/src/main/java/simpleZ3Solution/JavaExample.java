...
class JavaExample {
	...
	public void parserExample1(Context ctx) throws TestFailedException {
		...
		String test = "(declare-const x1 Int)(declare-const x2 Int)(declare-const x3 Int)(declare-const sum Int)(assert (and (and (>= x1 1) (<= x1 3))(and (>= x2 1) (<= x2 3))(and (>= x3 1) (<= x3 3))(and (not (and (= x1 x2)(= x1 x3))))(= sum (+ x1 x2 x3))))";
		//test1 optimization case does not work but it works in rise4fun
		String test1 = "(declare-const x1 Int)"
					+ "(declare-const x2 Int)"
					+ "(declare-const x3 Int)"
					+ "(assert (and (and (>= x1 1)(<= x1 3))"
					+ "			 (and (>= x2 1)(<= x1 3))"
					+ "			 (and (>= x3 1)(<= x1 3))"
					+ "			 (not (and (= x1 x2)(= x1 x3)))"
					+ "		)		"	 
					+ ")"
					+ "(maximize (+ (ite (= x1 x2) 0 " + intensity[0][1]+ ") (ite (= x2 x3) 0 "+ intensity[1][2] + ") ))";
		//test2 optimization case
		String test2 = "(declare-const x Int)"
					+ "(declare-const y Int)"
					+ "(assert (and (< x 4) (> y 1) (< (- y x) 1) ))"
					+ "(maximize (- y x))";
		
		String test3 = "(declare-const x Int)"
				+ "(declare-const y Int)"
				+ "(assert (and(< x 2)(< (- y x) 1)))"
				+ "(maximize (+ x y))";
		BoolExpr f = ctx.parseSMTLIB2String(test2, null, null, null, null)[0];
	...
	public void parserExample2(Context ctx) throws TestFailedException {
		...
		BoolExpr f = ctx.parseSMTLIB2String("(assert (and(> a b) (> a 6)))", null, null, declNames, decls)[0];
		System.out.println("formula: " + f);
		Model m = check(ctx, f, Status.SATISFIABLE);
		System.out.println("model:" + m);
